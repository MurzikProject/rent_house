...
print('Accuracy of Ridge classifier on test set: {:.2f}'.format(ridge.score(X_test, y_test)))
...
# Построим ROC-кривую
ridge_roc_auc = roc_auc_score(y_test, ridge.predict(X_test))
fpr, tpr, thresholds = roc_curve(y_test, ridge.predict_proba(X_test)[:,1])
plt.figure()
plt.plot(fpr, tpr, label='Ridge Classifier (area = %0.2f)' % ridge_roc_auc)
plt.plot([0, 1], [0, 1],'r--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate')
